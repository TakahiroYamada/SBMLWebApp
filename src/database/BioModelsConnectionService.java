...
public class BioModelsConnectionService implements AutoCloseable {
  ...
  private CuratedModelsResponse doGetAllCuratedModels(CuratedModelsResponse firstPage) {
    ...
    Stream<ModelSummary> remainder = IntStream.range(1, pageCount).mapToObj(this::getSearchResultsPage).filter(p -> p != null) // requests
                                                                                                                               // that
                                                                                                                               // threw
                                                                                                                               // exceptions
                                                                                                                               // return
    ...
    Set<ModelSummary> responses = Stream.concat(firstPage.getModelsStream(), remainder).collect(Collectors.toCollection(TreeSet::new));
  ...
  private <T> T performRequest(HttpGet request, AbstractResponseHandler<T> responseHandler) {
    try {
      ...
      System.err.printf("I/O exception encountered while performing request %s: %s%n", request.getURI().toString(), e);
    }
    return null;
  }
